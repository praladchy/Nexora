...
const StatGrid = () => {
  ...
  const totalOrders =
    orderaggregateData?.data?.OrderStatus?.totalOrderproducts || 0;
  const totalCancelledOrders =
    orderaggregateData?.data?.OrderStatus?.totalCancelledOrderProducts || 0;
  const totalPendingOrders =
    orderaggregateData?.data?.OrderStatus?.totalPendingOrderProducts || 0;
  const totalSale = orderaggregateData?.data?.OrderStatus?.totalPrice || 0;

  console.log("poiuy22", totalOrders);
  console.log("poiuysd", categoryAggregateData);
  console.log("poiuyqw", DataAggregate);

  const todaySales = DataAggregate?.orderInDay?.[0]?.TotalSales || 0;
  console.log("poiuyqw1", todaySales);
  const TodayOrders = DataAggregate?.orderInDay?.[0]?.TotalOrder || 0;
  console.log("poiuyqw2", TodayOrders);
  const totalProductsTodaySales =
    DataAggregate?.orderInDay?.[0]?.TotalProducts || 0;
